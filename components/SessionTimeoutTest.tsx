...
export default function SessionTimeoutTest() {
  ...
  if (status === "unauthenticated") return <div>Não conectado</div>;
  ...
  return (
    <div className="fixed top-4 right-4 bg-red-100 border border-red-400 text-red-700 px-4 py-2 rounded z-50">
      <div className="text-sm font-bold">Teste de expiração de sessão (30s)</div>
      <div className="text-xs">
        {isActive ? `Tempo restante: ${formatTime(timeLeft)}` : 'Sessão inativa'}
      </div>
      <div className="text-xs">
        Status: {status}
      </div>
