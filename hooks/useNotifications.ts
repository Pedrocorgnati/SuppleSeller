...
export const useNotifications = () => {
  ...
  const getCurrentUserId = useCallback(async () => {
    if (!session?.user?.email) return "mock-user";
    
    try {
      const baseUrl = process.env.NEXT_PUBLIC_API_BASE_URL;
      if (!baseUrl) return "mock-user";

      const response = await fetch(`${baseUrl}/api/users/email/${session.user.email}`);
...
export const useUnreadCount = () => {
  ...
  const fetchUnreadCount = useCallback(async () => {
    ...
    try {
      const baseUrl = process.env.NEXT_PUBLIC_API_BASE_URL;
      const userId = baseUrl
        ? (await (await fetch(`${baseUrl}/api/users/email/${session.user.email}`)).json())?.id
        : "mock-user";
